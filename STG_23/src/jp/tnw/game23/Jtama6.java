...
public class Jtama6 extends Jtama{
 ...
 //加速度が初速に影響を与える------------------
		...
		if(No>3)No=0;//ループアニメ*/
			...
			if(No[i]<4.5){No[i]=No[i]+0.017*15;//時間の概念の共有
			if(No[i]>4.5)No[i]=0;
    ...
    public int Hit(double x,double y){
		...
		{if(Flag[i]!=0){//リクエストされたものは移動
			...
			double x_sa=(X[i]+50)-(x+20);//ｘ、ｙは中心じゃないです
			double y_sa=(Y[i]+50)-(y+20);//ちゃんとｘ+ｒ、ｙ+ｒで円心を計算する
			double D=Math.sqrt(x_sa*x_sa+y_sa*y_sa);
				if(D<=20+50){//青たま半径は50、緑たま半径は20;D<=r+R 接触
					//Xsp[i]=0;
					//Ysp[i]=0;
					X[i]=20000;
					Flag[i]=0;
...
g.drawImage(Jtama,
				(int)X[i],(int)Y[i],(int)X[i]+16+100,(int)Y[i]+16+100,//画面のどこに?x,y左上座標　右下座標
				48*(int)No[i],140,48*((int)No[i]+1),192,//画像のどこか?x,y左上座標　右下座標
				Wind);//座標は小数ではダメ
