...
public class enemyAttcak {
	...
	//ﾘｸｴｽﾄ(上下弾一発)
	//-------------------------
	public void Req(double x, double y,int n){
		...
		for(int i=0;i<Max;i++){
			if(Flag[i]==0){
				...
				if(AngleData[n][count]==999)break;//空き敵を使いきらないためfor文を途中で抜ける
...
//			}
	...
	//円軌道--------------------------------
			...
			//画面外処理--------------------------------
			if(X[i]<-48||X[i]>960+48||Y[i]<-48||Y[i]>540+48){Flag[i]=0;}
			//画面外のキャラクタを待機状態にさせる
		...
		if(No>3)No=0;//ループアニメ*/
			...
			if(No[i]<9.5){No[i]=No[i]+0.017*10;//時間の概念の共有
			if(No[i]>9.5)No[i]=0;
	...
	public  void print(Graphics2D 	g ,JFrame	Wind) {//disp
		...
		{
			if(Flag[i]!=0){
		g.drawImage(enemyAttcak,
				(int)X[i],(int)Y[i],(int)X[i]+16,(int)Y[i]+16,//画面のどこに?x,y左上座標　右下座標
				16*(int)No[i],16,16*((int)No[i]+1),16+16,//画像のどこか?x,y左上座標　右下座標
				Wind);//座標は小数ではダメ
