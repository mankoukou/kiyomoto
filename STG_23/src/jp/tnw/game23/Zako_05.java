...
public class Zako_05 extends Zako_01 {
			...
			public void Req3(){
				...
				if(c==Max){//次の軌道の初期化
					...
					for( i=0;i<Max;i++){
							...
							Xsp[i]=200;
							Ysp2[i]=100;
							XX[i]=50*Math.cos(kaku[i]*Math.PI/180.0)-50*Math.sin(kaku[i]*Math.PI/180.0)+200;
							YY[i]=50*Math.sin(kaku[i]*Math.PI/180.0)+50*Math.cos(kaku[i]*Math.PI/180.0)+100+Y2[i];
	...
	case 1:
		...
		if(YY[i]>100)Flag[i]=2;
		break;	
	case 2:
		
		
		XX[i]=50*Math.cos(kaku[i]*Math.PI/180.0)-50*Math.sin(kaku[i]*Math.PI/180.0)+200;
		YY[i]=50*Math.sin(kaku[i]*Math.PI/180.0)+50*Math.cos(kaku[i]*Math.PI/180.0)+100;
